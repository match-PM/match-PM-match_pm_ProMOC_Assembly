...
# A sentinel value used in 6DOF requests to indicate that a specific axis should not be changed.
NO_CHANGE_SENTINEL = -999999.0

# A hardware-specific scaling factor required by the PMC library for rotary motion commands.
PMC_ROTARY_SCALING_FACTOR = 10.0
...
class ServiceCallbacks:
    ...
    def _process_6dof_input(self, request, current_position: list) -> list:
        """Process 6DOF input with a special value for 'no change'."""
        return [
            request.x_pos / 1000.0 if request.x_pos != NO_CHANGE_SENTINEL else current_position[0],
            request.y_pos / 1000.0 if request.y_pos != NO_CHANGE_SENTINEL else current_position[1],
            request.z_pos / 1000.0 if request.z_pos != NO_CHANGE_SENTINEL else current_position[2],
            math.radians(request.rx_pos) if request.rx_pos != NO_CHANGE_SENTINEL else current_position[3],
            math.radians(request.ry_pos) if request.ry_pos != NO_CHANGE_SENTINEL else current_position[4],
            math.radians(request.rz_pos) if request.rz_pos != NO_CHANGE_SENTINEL else current_position[5]
        ]
    ...
    def callback_rotary_motion(self, request, response):
        ...
        try:
            ...
            # The PMC library requires the rotary command to be scaled by a specific factor.
            scaled_target_rz = target_rz_rad / PMC_ROTARY_SCALING_FACTOR
