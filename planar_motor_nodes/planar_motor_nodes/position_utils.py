...
class PositionUtils:
    """Utility-Klasse für Position-Management, entkoppelt vom ROS-Node."""

    def __init__(self, logger, pmc_interface: PmcInterface, config: NodeConfig):
        """Initialisiert die Klasse mit expliziten Abhängigkeiten."""
        self.logger = logger
    ...
    def get_current_position(self, xbot_id: int = 0) -> Optional[List[float]]:
        """Holt die aktuelle XBot-Position über das PmcInterface."""
        try:
    ...
    def get_xbot_status_info(self, xbot_id: int = 0) -> Optional[dict]:
        """Holt umfassende XBot-Statusinformationen."""
        try:
    ...
    def get_xbot_state_string(self, xbot_id: int = 0) -> str:
        """Holt nur den XBot-Status als String."""
        try:
            xbot_status = self.pmc.bot.get_xbot_status(xbot_id)
    ...
    def _xbot_state_to_string(self, xbot_state) -> str:
        """Konvertiert den XbotState-Enum in einen lesbaren String."""
        try:
    ...
    def is_position_in_bounds(self, x: float, y: float, z: float) -> bool:
        """Prüft, ob eine Position innerhalb der definierten Grenzen liegt."""
    ...
    def diagnose_xbot_availability(self) -> dict:
        """Diagnostiziert, welche XBots verfügbar sind und antworten."""
