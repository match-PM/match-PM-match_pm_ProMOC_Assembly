...
def test_camera_node_publishes_canonical_topics_only():
    ...
    assert "/promoc/camera/image_raw" in node_content
    assert "/promoc/camera/status" in node_content
...
def test_removed_runtime_paths_are_not_imported():
    ...
    assert "from .drivers.mock import MockCameraDriver" in node_content
    assert "from .drivers.hardware import HardwareCameraDriver" in node_content
