...
class AutofocusHandler(CallbackBase):
    ...
    def autofocus_callback(self, request, response):
        ...
        if skip_flyover:
            ...
            peak_start, peak_end, max_stddev = self._fly_over_detection(
                request.start_position, request.end_position, clients, focus_profile
            )
    ...
    def _get_all_axis_clients(self):
        ...
        clients = {
            "move": self._node.create_client(
                MoveAbsolute, f"/{x_axis_name}/move_absolute"
            ),
            "jog": self._node.create_client(JogAxis, f"/{x_axis_name}/jog_axis"),
            "status": self._node.create_client(
                GetOperationStatus, f"/{x_axis_name}/get_operation_status"
            ),
            "position": self._node.create_client(
                GetPosition, f"/{x_axis_name}/get_position"
            ),
            "stop": self._node.create_client(Stop, f"/{x_axis_name}/stop"),
            "get_vel": self._node.create_client(
                GetVelocityParameters, f"/{x_axis_name}/get_velocity_parameters"
            ),
            "set_vel": self._node.create_client(
                SetVelocityParameters, f"/{x_axis_name}/set_velocity_parameters"
            ),
        }
    ...
    def _get_position(self, clients) -> float:
        ...
        if (
            hasattr(self._node, "current_axis_position")
            and self._node.current_axis_position >= 0
        ):
    ...
    def _run_single_mode(
        ...
        refinement_shrink_factor = self._param_float(
        ...
        min_step = float(
            (focus_profile or {}).get(
                "min_step_mm", self._param_float("autofocus.min_step_mm", 0.01)
            )
        )
        ...
        if save_best_image:
            best_position, best_score, measurements, best_image = (
                self._run_autofocus_loop(
                    af, clients, return_best_image=True, settle_s=settle_s
                )
            )
        else:
            best_position, best_score, measurements = self._run_autofocus_loop(
                af, clients, settle_s=settle_s
            )
            best_image = None
        ...
        response.status_message = (
            f"{mode_name}: pos={best_position:.3f}mm, score={best_score:.0f}"
        )
        ...
        try:
            ...
            response.measurement_positions = [
                float(m.position_mm) for m in measurements
            ]
    ...
    def _run_autofocus_loop(
        ...
        try:
            ...
            if step_size and af.config.end_mm > af.config.start_mm:
                max_steps = max(
                    max_steps,
                    int((af.config.end_mm - af.config.start_mm) / float(step_size)) + 5,
                )
        except Exception:
            pass
        ...
        for _ in range(max_steps):
            ...
            cv_image, ts = self._wait_for_new_image(
        ...
        if (
            ...
            and hasattr(af, "_best_measurement")
            and af._best_measurement
        ):
    ...
    def _run_comparison(
        ...
        for mode, name, algo_class in AUTOFOCUS_ALGORITHMS:
            ...
            best_pos, best_score, measurements = self._run_autofocus_loop(
                af, clients, settle_s=settle_s
            )
