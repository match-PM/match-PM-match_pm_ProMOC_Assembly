...
def analyze_field_curvature(
    ...
    """
    from .mtf import MTFAnalyzer, MTFConfig
    ...
    # Configure analyzer
    config = MTFConfig(
        pixel_size_um=pixel_size_um, roi_width=roi_size, roi_height=roi_size
    )
...
def analyze_field_curvature_detailed(
    image: np.ndarray, pixel_size_um: float = 2.40, roi_size: int = 200
) -> Dict[str, dict]:
    """
...
def calculate_field_metrics(field_data: Dict[str, float]) -> dict:
    ...
    if not corners:
        return {"center_mtf50": center, "error": "No corner measurements"}
