...
- `AxisClientManager`: talks to the linear-axis services
- `AutofocusRunner`: executes one autofocus run end-to-end
- `AutofocusHandler`: owns ROS request/response orchestration

This keeps the maintained Four-Step path understandable without turning the
service callback into one large function.
"""
...
<<<<<<< HEAD
        ...
        maintained autofocus path. The wire-level ``focus_mode`` field remains
        available for transition compatibility, but every mode value is mapped
        to the same implementation so callers continue to work while the branch
        presents one clear autofocus workflow.
        """
        if int(mode) not in _FOUR_STEP_MODE_IDS:
