...
Usage:
    from camera_nodes.algorithms.synthetic_targets import generate_slanted_edge
    
    # Generate perfect edge with 5° slant
...
def generate_square_target(
    ...
    if cv2 is not None:
        ...
        image[center - half_sq:center + half_sq,
              center - half_sq:center + half_sq] = 255
    
    return image


def add_noise(image: np.ndarray, noise_type: str = 'gaussian', level: float = 0.05) -> np.ndarray:
    """
...
def validate_mtf_algorithm(
    ...
    # Test 1: Perfect edge
    spec1 = generate_slanted_edge_with_spec(angle=5.0, blur_sigma=0.0, pixel_size_um=pixel_size_um)
    result1 = analyzer.compute_mtf(spec1.image)
    
    if result1.valid:
        error_pct = abs(result1.mtf50 - spec1.expected_mtf50) / spec1.expected_mtf50 * 100
        ...
        results['tests'].append({
            'name': 'Perfect Edge',
            'passed': False,
            'error': result1.error_msg
        })
    
    # Test 2: Blurred edge
    spec2 = generate_slanted_edge_with_spec(angle=5.0, blur_sigma=1.0, pixel_size_um=pixel_size_um)
    result2 = analyzer.compute_mtf(spec2.image)
    
    if result2.valid:
        error_pct = abs(result2.mtf50 - spec2.expected_mtf50) / spec2.expected_mtf50 * 100
        test_passed = error_pct < tolerance_percent
