...
class VerificationCallbacks(CallbackBase):
    ...
    def verify_autofocus_callback(self, request, response):
        ...
        except (ConfigurationError, ServiceCallFailedError) as e:
            response.success = False
            response.status_message = f'Verification failed: {str(e)}'
            self._node.get_logger().error(response.status_message)
            
        except Exception as e:
            response.success = False
            response.status_message = f'Unexpected error: {str(e)}'
            self._node.get_logger().error(response.status_message, exc_info=True)
    ...
    def verify_mtf_callback(self, request, response):
        ...
        try:
            ...
            response.success = False
            response.status_message = f'MTF verification failed: {str(e)}'
            self._node.get_logger().error(response.status_message)
