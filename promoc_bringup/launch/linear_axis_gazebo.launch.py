#!/usr/bin/env python3
# filepath: /home/soc/Development/Ros2/promoc_assembly/src/match-PM-match_pm_ProMOC_Assembly/promoc_bringup/launch/linear_axis_gazebo.launch.py
...
def generate_launch_description():
    ...
    if ros_distro == 'humble':
        xacro_file = os.path.join(
            promoc_bringup_dir, 'urdf', 'linear_axis_humble.urdf.xacro')
    else:
        xacro_file = os.path.join(
            promoc_bringup_dir, 'urdf', 'linear_axis_jazzy.urdf.xacro')
    ...
    if ros_distro == 'humble':
        ...
        spawn_entity = Node(
            package='ros_gz_sim',
            executable='create',
            arguments=['-topic', 'robot_description', '-name', 'linear_axis'],
            output='screen',
        )

    # Joint State Broadcaster (starten nach Gazebo)
    load_joint_state_broadcaster = TimerAction(
        period=3.0,  # Warte 3 Sekunden nach Gazebo-Start
        actions=[
            Node(
    ...
    # Position Controllers für X und Z Achse
    load_x_position_controller = TimerAction(
        period=4.0,  # Nach Joint State Broadcaster
        actions=[
            Node(
                package='controller_manager',
                executable='spawner',
                arguments=['linear_x_joint_position_controller',
                           '--controller-manager', '/controller_manager'],
                output='screen',
            )
        ]
    ...
    load_z_position_controller = TimerAction(
        period=4.5,  # Leicht verzögert
        actions=[
            Node(
                package='controller_manager',
                executable='spawner',
                arguments=['linear_z_joint_position_controller',
                           '--controller-manager', '/controller_manager'],
                output='screen',
            )
        ]
    ...
    lts300_x_axis_node = TimerAction(
        period=6.0,  # Nach Controller-Setup
        actions=[
            Node(
                ...
                parameters=[{
                    'use_sim_time': use_sim_time,
                    'use_gazebo': True,  # ✅ Gazebo-Integration aktivieren
                    'debug_mode': debug_mode,
                    'x_axis_serial': 'SIM_X',  # ✅ Simulation Serial
                    'z_axis_serial': 'NONE',   # Nur X-Achse
                ...
                output='screen',
                remappings=[
                    # Remap für konsistente Topic-Namen
    ...
    lts300_z_axis_node = TimerAction(
        period=6.5,  # Leicht verzögert nach X-Achse
        actions=[
            Node(
                package='linear_axis_nodes',
                executable='lts300_service_node',
                ...
                parameters=[{
                    ...
                    'use_gazebo': True,  # ✅ Gazebo-Integration aktivieren
                    'debug_mode': debug_mode,
                    'x_axis_serial': 'NONE',   # Nur Z-Achse
