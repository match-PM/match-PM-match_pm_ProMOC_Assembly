...
class SimulatedLinearAxisDriver(LinearAxisDriver):
    ...
    def move_absolute(self, position: float):
        ...
        self._move_duration = abs(
            position - self._position) / 100.0 + 0.5  # Min 0.5s
    ...
    def get_position(self) -> float:
        if self._is_moving:
            ...
            if elapsed_time >= self._move_duration:
                ...
                start_pos = self._position if hasattr(
                    self, '_start_position') else 0.0
                self._position = start_pos + \
                    (self._target_position - start_pos) * progress
    ...
    def validate_position(self, position: float) -> bool:
        ...
        min_position = 0.0
        max_position = 300.0  # LTS300 has 300mm travel
