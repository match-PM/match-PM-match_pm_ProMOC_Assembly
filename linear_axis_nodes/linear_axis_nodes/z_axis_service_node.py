...
from Thorlabs.MotionControl.IntegratedStepperMotorsCLI import * #type: ignore[import]
...
class Z_Axis_Service_Node(Node): 
    def __init__(self):
        ...
        self.declare_parameter('serial_no', '45318394')
        self.serial_no = self.get_parameter('serial_no').get_parameter_value().string_value
        self.get_logger().info(f"Using device with serial number: {self.serial_no}")
    ...
    def startup_connection(self):
        try:
            # create new device
            print(f"Attempting to connect to device with serial number: {self.serial_no}")
            ...
            print(f"Device with serial number {self.serial_no} is not ready.")
            return
        except Exception as e:
