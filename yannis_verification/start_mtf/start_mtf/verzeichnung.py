#!/usr/bin/env python3
"""
Wissenschaftlich korrektes Verzeichnungsmessungssystem nach DIN/ISO-Standard.
Inklusive SVG-Export, deutschem CSV-Format und Sensor-Visualisierung.
Unterstützt Schnurrbart-Verzeichnung (Mustache Distortion).
"""
...
class HeadlessScientificAnalyzer(Node):
    ...
    def __init__(self):
        ...
        self.save_dir = "/home/pmlab/Dokumente/Messungen/Yannis Wesser/Verzeichnung"
        ...
        self.TOTAL_MEASUREMENTS_NEEDED = 5
    ...
    def generate_plots_and_tables(self):
        ...
        coeffs, _, _, _ = np.linalg.lstsq(X_poly * w[:, None], d_fit * w, rcond=None)
        k1, k2, k3= coeffs
        ...
        # Symmetrische x-Achse für Schnurrbart-Darstellung
        x_abs_max = max(0.2,
                        abs((mu - s3)).max() * 1.1,
                        abs((mu + s3)).max() * 1.1,
                        abs(d_poly).max() * 1.1)
        ...
        def setup_plot(title):
            ...
            ax.axvspan(x_min, 0, color='#FFF3CD', alpha=0.5, label='Tonnen-Verzeichnung')
            # Pincushion-Region (positiv) = Kissenverzeichnung
            ax.axvspan(0, x_max, color='#D4EDDA', alpha=0.5, label='Kissen-Verzeichnung')
            ax.axvline(0, color='black', linestyle='-', linewidth=1.0)
            ax.set_xlim(x_min, x_max)
            ax.set_ylim(0, y_max)
            ax.set_title(title, fontsize=11, fontweight='bold')
            ax.set_xlabel('Relative radiale Verzeichnung $D(r)$ [%]', fontsize=10)
            ax.set_ylabel('Feldhöhe $r$ [mm]', fontsize=10)
        ...
        # PLOT 1: Nur Mittelwert
        fig1, ax1 = setup_plot('Verzeichnung - Mittelwert (n=100)')
        fig1.set_size_inches(11, 6)
        ax1.plot(mu, fh, 'o-', color='navy', markersize=5, linewidth=2.0,
                 label='Kurve der Mittelwert')
        for r_z in zero_crossings:
            ax1.axhline(r_z, color='darkorange', linestyle=':', linewidth=1.5,
                        label=f'Vorzeichenwechsel r = {r_z:.3f} mm')
        ax1.legend(loc='upper left', fontsize=9)
        fig1.tight_layout()
        fig1.savefig(os.path.join(self.save_dir, "plot_01_mittelwert.svg"), format='svg')
        plt.close(fig1)

        # PLOT 2: Nur Polynomfit
        fig2, ax2 = setup_plot('Verzeichnung - Polynomfit (n=100)')
        ...
        fig2.tight_layout()
        fig2.savefig(os.path.join(self.save_dir, "plot_02_polynom.svg"), format='svg')
        plt.close(fig2)

        # PLOT 3: Kombiniert (Alle)
        fig3, ax3 = setup_plot('Verzeichnung — Mittelwert, ± 3σ und Polynomfit (n=100)')
        
        # JETZT NEU: Setzt die Bildgröße explizit auf breit (Breite=11, Höhe=6 Zoll)
        fig3.set_size_inches(11, 6)

        ax3.fill_betweenx(fh, mu - s3, mu + s3, color='royalblue', alpha=0.15,
                          label='3 $\sigma$ (99,7 %)')
        ax3.plot(mu, fh, 'o-', color='navy', markersize=4, linewidth=1.5,
                 label='Kurve der Mittelwert')
        ax3.plot(d_poly, r_dense, '--', color='crimson', linewidth=2.0,
                 label=poly_label_clean)
        for r_z in zero_crossings:
            ax3.axhline(r_z, color='darkorange', linestyle=':', linewidth=1.5,
                        label=f'Vorzeichenwechsel r = {r_z:.3f} mm')
        
        # JETZT NEU: Legende oben links platziert
        ax3.legend(loc='upper left', fontsize=9)
        fig3.tight_layout()
        
        fig3.savefig(os.path.join(self.save_dir, "plot_03_kombiniert.svg"), format='svg')
        fig3.savefig(os.path.join(self.save_dir, "plot_03_kombiniert.png"), format='png', dpi=300)
        plt.close(fig3)
