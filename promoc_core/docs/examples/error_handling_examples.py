...
from promoc_core.error_handling import handle_service_errors, ServiceResponse
...
class ExampleNode(Node):
    ...
    def move_absolute_callback(self, request, response):
        ...
        self.driver.move_absolute(position=request.position, timeout=request.timeout)
...
class NodeWithRecovery(Node):
    ...
    def perform_operation_with_recovery(self, position: float):
        ...
        try:
            ...
            # Attempt recovery
            context = {"driver": self.driver, "requested_position": position}
...
class BestPracticesExample:
    ...
    def specific_exception_handling(self, position: float):
        ...
        try:
            ...
            safe_position = max(
                self.driver.min_position, min(position, self.driver.max_position)
            )
